...
int main() {
    ...
    while (!is_end) {
        ...
        for (plant_t plant : plants) {
            ...
            if (plant.type == POTATO && plant.is_alive) {
                for (zombies_t& zombie : zombies) {
            ...
            for (int i = plants.size() - 1; i >= 0; i--) {
                if (plants[i].type == POTATO && plants[i].is_alive) {
        ...
        for (zombies_t& zombie : zombies) {
            ...
            // make potato alive
            for (plant_t& plant : plants) {
                if (plant.type == POTATO && plant.y >= next_pos && plant.x == zombie.x) {
                    plant.is_alive = true;
                }
            }
            zombie.y = next_pos;
